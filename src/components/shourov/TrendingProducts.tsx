...
import { getFoods } from "@/src/lib/foods";
import { useEffect, useState } from "react";
import { ProductType } from "@/src/types/foods";

const TrendingProducts = () => {
	const [showFoods, setShowFoods] = useState<ProductType[]>([]);
  ...
  const [foodData, setFoodData] = useState<ProductType[]>([]);
	...
	useEffect(() => {
      ...
      const dogFood = foodData.filter(
        (item) => item.category.toLowerCase() === "Birds".toLowerCase(),
      );
		setShowFoods(dogFood);
	}, [showFoods]);


	// const catFood = foodData.filter((item) => item.category === "Cat");
	return (
		<section>
				...
				<div className="container">
					...
					<div className="pt-[20px] lg:pt-[50px] flex items-center justify-between flex-wrap gap-y-3">
						...
						<button className="cursor-pointer flex items-center gap-2">
							...
							<Image src={arrow_right} width={24} height={14} alt="icons" />
						</button>
					</div>
